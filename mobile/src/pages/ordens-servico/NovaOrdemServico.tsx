...
import { getServicos, criarOrdemServico } from '../../services/api';
...
const NovaOrdemServico: React.FC = () => {
  ...
  // Trecho atualizado da função handleConfirmar no NovaOrdemServico.tsx
...
const handleConfirmar = async () => {
  ...
  try {
    ...
    if (response && response.id) {
      ...
      history.push(`/ordens-servico/${response.id}/esteira`);
    } else {
      history.push('/ordens-servico/hoje');
    }
  } catch (e: unknown) {
    const msg = e instanceof Error ? e.message : "Erro ao iniciar ordemServico";
    setToastMsg(msg);
    setShowToast(true);
  } finally {
    setLoading(false);
