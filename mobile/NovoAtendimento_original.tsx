...
const NovoAtendimento: React.FC = () => {
  ...
  const servicosModelo = useMemo<Servico[]>(() => [
    ...
    { id: 3, nome: 'Detalhe Interno (Higieniza├º├úo)', duracao_estimada_min: 120, preco: '250.00' },
    { id: 4, nome: 'Polimento T├®cnico', duracao_estimada_min: 180, preco: '450.00' }
  ], []);
  ...
  const handleConfirmar = async () => {
    if (!placa || !modelo || !servicoSelecionado || (isAgendar && !horarioSelecionado)) {
      alert("Por favor, preencha todos os campos e escolha o hor├írio.");
      return;
    }

    const dadosAtendimento = {
      nome_dono: "Cliente Balc├úo", 
      celular_dono: "",
      placa: placa.toUpperCase(),
      modelo: modelo,
      marca: "N├úo informada",
      cor: "",
        ...
        <div style={{ background: '#000', minHeight: '100vh', padding: '32px 20px 140px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              {/* Cont├¬iner da Logo Substituindo o s├¡mbolo de '+' */}
          ...
          <h2 style={{ color: '#fff', fontSize: '32px', fontWeight: 900, margin: '0 0 4px', letterSpacing: '-1px' }}>
            {isAgendar ? 'Agendar Servi├ºo' : 'Entrada R├ípida'}
          </h2>
          <p style={{ color: '#444', fontSize: '15px', fontWeight: 700, marginBottom: '32px' }}>
            {isAgendar ? 'Selecione data e hor├írio' : 'Iniciar atendimento imediato'}
          </p>

          {/* ... restante do formul├írio (placa, modelo, servi├ºos, hor├írios) ... */}
          ...
          <label style={{ color: '#fff', fontSize: '12px', fontWeight: 900, textTransform: 'uppercase', display: 'block', marginBottom: '16px' }}>Servi├ºo</label>
          ...
          {isAgendar && servicoSelecionado && (
            <div style={{ marginBottom: '40px' }}>
              <label style={{ color: '#fff', fontSize: '12px', fontWeight: 900, textTransform: 'uppercase', display: 'block', marginBottom: '12px' }}>Data e Hor├írios Dispon├¡veis</label>
              <input 
          ...
          {/* TabBar Inferior Padronizada (Mantida conforme o padr├úo visual f3192f.png) */}
          <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, height: '100px', background: 'rgba(12,12,12,0.98)', backdropFilter: 'blur(20px)', borderTop: '1px solid #1a1a1a', display: 'flex', justifyContent: 'space-around', alignItems: 'center', padding: '0 10px 20px', zIndex: 100 }}>
            ...
            <div onClick={() => history.push('/atendimentos/historico')} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px', flex: 1, color: '#444', cursor: 'pointer' }}>
              ...
              <span style={{ fontSize: '9px', fontWeight: 900, textTransform: 'uppercase' }}>Hist├│rico</span>
            </div>
          </div>
