...
> - **Gestão B2B e Geolocalização:** Ao sanitizar os campos da API de Gestão unificada (`GestaoViewSet`, `/api/gestao/estabelecimento/`), os campos `latitude` e `longitude` são MANDATÓRIOS na whitelist de `campos_permitidos` do PATCH e nas respostas GET/PATCH, suportando a comunicação bidirecional de pinos geográficos, que devem ser integrados e protegidos contra CORS.
