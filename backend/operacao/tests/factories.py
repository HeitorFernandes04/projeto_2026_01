...
class VeiculoFactory(factory.django.DjangoModelFactory):
    ...
    estabelecimento = factory.SubFactory(EstabelecimentoFactory)
    placa = factory.Sequence(lambda n: f'AAA0{n:02d}')
    modelo = 'Gol'
    marca = 'VW'
    cor = 'Branco'
...
class IncidenteOSFactory(factory.django.DjangoModelFactory):
    ...
    ordem_servico = factory.SubFactory(OrdemServicoFactory)
    tag_peca = factory.SubFactory(TagPecaFactory, estabelecimento=factory.SelfAttribute('..ordem_servico.estabelecimento'))
    descricao = 'Risco profundo detectado no para-choque.'
