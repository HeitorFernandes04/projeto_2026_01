...
1. Use a ferramenta MCP `mcp_lava-me-context-server_list_mobile_components` (ou `mcp_lava-me-context-server_list_web_components` se aplicável) para mapear o que foi implementado. Lembre-se de verificar os componentes no diretório `mobile-cliente` para features B2C.
...
2. Verifique se o Mobile B2C está chamando exclusivamente rotas `/api/cliente/` ou `/api/publico/` e o Web/Mobile B2B está usando `/api/gestao/`.
3. Qualquer chamada genérica (`/api/operacao/` ou legado) ou chamada cruzada de perfis deve gerar um alerta de Risco de Arquitetura.
