...
* **Mocks e Factories:** Utilize bibliotecas como `factory_boy` para gerar dados de teste. Evite acoplamento excessivo com o banco de dados se puder testar a lógica isoladamente.
...
* **Views Baseadas em Classes:** Utilize as *Generic API Views* ou *ViewSets* do DRF. NUNCA utilize views baseadas em templates (ex: `ListView`, `TemplateView`) ou retorne HTML.
...
* **Controle de Acesso Modular:** NUNCA faça verificações manuais de posse dentro dos métodos da View (ex: `if obj.dono != request.user:`). Você deve criar e/ou utilizar classes em `permissions.py` (ex: `IsGestor`, `IsClienteDonoDoVeiculo`) e aplicá-las em `permission_classes`.
* **Autenticação:** Assuma que estamos utilizando JWT (`djangorestframework-simplejwt`).
...
* **Arquivos e Mídia:** NUNCA salve arquivos localmente em views. Assuma o uso de `django-storages` (S3).
* **URLs e Rotas:** NUNCA faça hardcode de URLs. Utilize o sistema de roteamento dinâmico do Django. Toda rota deve estar versionada (ex: `api/v1/...`).
